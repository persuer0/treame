...
HCanvas.prototype.drawContour = function (points, option) {
  option = option || {};

  var context = this.context;
  var lineWidth = option.lineWidth || 3;
  var strokeColor = option.strokeColor || '#f00';
  var closed = typeof option.closed === 'undefined' ? true : option.closed;
  var minScore = typeof option.minScore === 'undefined' ? 0.3 : option.minScore;
  var validPoints = points.filter(item => item && item.score > minScore);
  ...
  for (var i = 1; i < validPoints.length; i++) {
    context.lineTo(validPoints[i].x, validPoints[i].y);
  }
