  ...
  points.map(item => {
    switch (item.type) {
      ...
      case "corner":
        ...
        break;
      case "lmk_106pts": {
        const outerLipIndices = [84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95];
        const innerLipIndices = [96, 97, 98, 99, 100, 101, 102, 103];
        const outerLipPoints = outerLipIndices.map(index => item.skeletonPoints[index]).filter(Boolean);
        const innerLipPoints = innerLipIndices.map(index => item.skeletonPoints[index]).filter(Boolean);

        this.smartCanvas1.drawArcPoint(item.skeletonPoints, "#0ff", item.diameterSize);
        this.smartCanvas1.drawContour(outerLipPoints, { strokeColor: "#f00", lineWidth: 2, closed: true });
        this.smartCanvas1.drawContour(innerLipPoints, { strokeColor: "#f00", lineWidth: 2, closed: true });
        break;
      }
      case "parking":
